...
main(int argc, char **argv)
{
	...
	std::ofstream listF("list_frames.txt");
	std::ofstream listFSaliency("list_frames_saliency.txt");
	...
	if((rows_rgb == rows_tmp)  && (cols_rgb==cols_tmp)){ //the frames are ready and already captured
         num_j= 0;
	      ...
	      num_i= 0;
	       while (num_i*CONST_STRIDE+CONST_PATCH < WIDTH ){
					 ...
					 cv::Mat region1 = frame_rgb.rowRange(num_j * CONST_STRIDE, num_j * CONST_STRIDE + CONST_PATCH).colRange(num_i * CONST_STRIDE, num_i * CONST_STRIDE + CONST_PATCH);
					 cv::Mat region2 = frame_tmp.rowRange(num_j * CONST_STRIDE, num_j * CONST_STRIDE + CONST_PATCH).colRange(num_i * CONST_STRIDE, num_i * CONST_STRIDE + CONST_PATCH);
					 ...
					 if(region2.channels() == 3)
					 	cv::cvtColor(region2, tempo, cv::COLOR_BGR2GRAY);
					 else
					 	tempo = region2;

					 cv::split(region1, channels);

					 channels.push_back(tempo);
	 ...
	 mix = kernelXY;
		...
		std::string filename = "../Data/ChaboNet_Saliency/Frames" + count_str.str() + extension;
		cv::imwrite(filename, frame_rgb);

		listFSaliency << "../../Lab4/Data/ChaboNet_Saliency/" << std::setw(8) << std::setfill('0') << currentProcessedFrames << extension << "\n";
		listF << "../../Lab4/Data/ChaboNet_Saliency/Frames/" << std::setw(8) << std::setfill('0') << currentProcessedFrames << extension << "\n";
